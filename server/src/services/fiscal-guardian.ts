...
// ============================================
// IA — Simulador de pérdida de certificación
// ============================================

export async function simulateCertificationLoss(tenantId: string) {
  const account = await getFiscalAccount(tenantId);
  const cert = await prisma.certificationProfile.findUnique({ where: { tenantId } });
...
- Costo adicional anual preliminar sin conversion: $${annualExtraCost.toLocaleString()} USD`,
  ...
  return {
    currentStatus: cert?.status || 'NOT_CONFIGURED',
    modality: cert?.modality || null,
    immediateImpact,
    monthlyExtraCost,
    annualExtraCost,
    cashFlowImpact,
