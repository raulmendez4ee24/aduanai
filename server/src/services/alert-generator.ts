  ...
  for (const s of statuses) {
    ...
    if (days < 0) {
      specs.push({
        ...
        content: `Tu inscripción venció el ${s.expirationDate.toLocaleDateString('es-MX')} (hace ${Math.abs(days)} días). El SAT puede embargar la mercancía y aplicar multa 70-100% del valor (Art. 144, 178 LA).`,
  ...
  if (blockedChecks.length > 0) {
    ...
    for (const [fraction, count] of fractionCounts) {
      ...
      specs.push({
        ...
        content: `Has intentado clasificar/cotizar la fracción ${fraction} ${count} veces en los últimos 14 días sin estar inscrito en: ${missing.join(', ')}. Si llegan a pedimento real, hay riesgo de embargo + multa 70-100%.`,
