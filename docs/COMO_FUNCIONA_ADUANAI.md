...
| COPILOT_CITA_ESTRICTA=estricta en prod + evidencia | 1 | CERRADO (27-ago, activo en Railway) | Al activarlo, "Explica la GRI 3" (sugerida) se DEGRADABA: el doc `GRI 1-6 LIGIE` no obtenía clave y 8/45 docs del corpus tampoco (rangos, listas, dobles `A · B`, sufijos). Fix: `clavesDeReferencia` (un doc = varias claves), rangos por pertenencia (RGI/Art.), Anexo del T-MEC en extractor y parser; `test:copilot-citas` 14/14; censo: 4 docs sin clave por diseño (Notas de Sección, CPTPP/TLCUEM enteros — formas que el extractor nunca emite ⇒ no pueden ser fantasma). Evidencia prod: ver cierre |
