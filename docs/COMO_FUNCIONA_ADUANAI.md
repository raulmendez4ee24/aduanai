...
| Clasificación de las 8 marcadas (fantasma real vs FP) | 1 | CERRADO (censo 25-ago) | 3 eran falsos positivos del matcher (Capítulo 5 TMEC, Arts. plural, "Art. 49 de la Ley" con cuerpo genérico → corregidos en `0a62cdd`); 5 fantasmas reales (Art. 59-II ×3, Art. 59 ×2, Art. 303/TMEC, Art. 24, Anexo 1) = artículos no presentes en el corpus → la abstención es correcta; estricta ahora los degrada en prod |
| Matcher: formas conocidas (Capítulo N TMEC, Arts. plural, cuerpo nulo) | 1 | CERRADO `0a62cdd` | `citas-legales.ts`: CUERPOS_GENERICOS + PATRON_LISTA_ARTS + capítulo T-MEC en ambos órdenes; `test:copilot-citas` 12/12 (verificado 27-ago contra `main` d1c04e6 + prod) |
| COPILOT_CITA_ESTRICTA=estricta en prod + evidencia | 1 | CERRADO (27-ago, activo en Railway; `4910755`+`9b216d0`+`8d9ecee`) | Activarlo destapó 3 defectos reales del matcher/flujo, corregidos con test: (1) docs con referencia compuesta sin clave (8/45: `GRI 1-6`, `Art. 73-89 LCE`, `Reglas 1.3.2 a 1.3.7`, `A · B`, `(automotriz)`, `— Textiles`) → `clavesDeReferencia` + rangos por pertenencia + Anexo T-MEC; (2) cita a doc que existe en corpus pero no fue recuperado (Art. 86-A LA) → recuperación POR CITA antes de regenerar (tope 4 docs); (3) ambigüedad medida por docs y no por preceptos distintos. `test:copilot-citas` 18/18. Evidencia prod (8d9ecee): las 3 sugeridas ×2 corridas = 6/6 sin degradar, citas respaldadas (`Art. 59 LA`, `Regla 1.3.1/1.3.6`, `GRI 1-6 LIGIE`, `Anexo 2.4.1`); 0 degradaciones en logs post-deploy. 4 docs siguen sin clave por diseño (Notas de Sección, CPTPP/TLCUEM enteros: el extractor nunca emite esas formas) |
| CLA_001 solo con reclasificaciones reales | 1 | CERRADO `0a62cdd` | `reclasificacionesReal(tenantId, fractionCode)`: where con tenantId + ventana 365 d + feedback='incorrect'; firma `{total, reclasificadas}` (verificado 27-ago contra `main` d1c04e6 + prod) |
| CLA_002 exige productDescription desde UI | 1 | CERRADO `0a62cdd` | GlosaSimulator.tsx captura `productDescription`; schema zod lo acepta (max 2000); sin descripción ⇒ regla no_evaluado (verificado 27-ago contra `main` d1c04e6 + prod) |
| REG_001 coherente IN/AF (muere "A4/diferir IVA") | 1 | CERRADO `0a62cdd` | seed `glosa-risk-rules.ts` reescrito a IN/AF; texto "A4/diferir IVA" eliminado (verificado 27-ago contra `main` d1c04e6 + prod) |
| ORI_002 con captura real de originCertificate | 1 | CERRADO `0a62cdd` | GlosaSimulator.tsx envía `documents.originCertificate` (checkbox real, línea 289) (verificado 27-ago contra `main` d1c04e6 + prod) |
| appliesTMEC valida membresía importando lista del Cotizador | 1 | CERRADO `0a62cdd` | `esMiembroTMEC` de `lib/treaties.ts` (TMEC_PAISES fuente única) usado en glosa-simulator.ts:406 (verificado 27-ago contra `main` d1c04e6 + prod) |
| Test transversal: 15 reglas vs campos capturados | 1 | CERRADO `0a62cdd` | `test:glosa-capturadas` 16/16 (verificado 27-ago contra `main` d1c04e6 + prod) |
| Seed DB limpia no resucita retiradas + test | 1 | CERRADO `0a62cdd` | `lib/retiradas-tigie.ts` + `activeParaSeed` en create y update; `test:seed-retiradas` 6/6 (verificado 27-ago contra `main` d1c04e6 + prod). Nota: tigie-data.ts:169 conserva la fila 85444201 como dato histórico pero el seed la crea inactiva |
...
| D10: una empresa demo, una historia | 1 | CERRADO `0a62cdd` | identidad única Maquiladora Ejemplo SA de CV (MEJ010203AB1) en todos los paths de seed; los otros nombres en demo-profiles.ts son perfiles sectoriales, no tenants; TC demo = tasa viva del sistema (`DEMO_FX_RESPALDO` 17.85 solo como último respaldo con warning); Pre-Glosa pre-llena IVA/IEPS desde CertificationProfile (verificado 27-ago contra `main` d1c04e6 + prod) |
...
| consultHash por tenant (hash+unicidad+upsert+feedback+test) | 3 | CERRADO en prod (`8caa578`, migración aplicada 27-ago) | hash incluye tenantId; `@@unique([tenantId, consultHash])`; feedback scopeado; `test:consult-hash` 7/7; en prod la misma consulta repetida pasa por el upsert sin unique violation |
...
| ClassificationKnowledge sin contaminación entre tenants | 3 | CERRADO en prod (`9b8050f`, migración aplicada 27-ago) | feedback crea filas con tenantId; retrieval consume solo `verified=true` o mismo tenant; `test:knowledge-tenant` 9/9 |
...
## Programa Operación 2026-08 — "de demo bonita a herramienta de trabajo" (27-ago-2026)

Ejecución completa del documento "Recomendaciones módulo por módulo, con ojos de operación". Plan, ramas y estado por ítem en `docs/PLAN_OPERACION_2026-08.md` (11 ramas integradas en `main`, todas VERIFICADAS con tsc + suites + build; deploy y verificación en prod al cierre). Resumen por módulo:

| Área | Qué se hizo | Pendiente de fuente oficial (etiquetado en UI) |
|---|---|---|
| Catálogo maestro de partes | `/catalogo`: parte = cliente→código→fracción→NICO→NOMs→dictamen versionado; Clasificador consulta el catálogo antes de correr; reclasificación con justificación y aprobación; Historial agrupado por producto, promover a catálogo, acierto por capítulo, Excel | — |
| Lote | `/clasificador/lote`: Excel/CSV → cola → semáforo verde/ámbar/rojo → Excel; adjuntos; uso/destino; subpartidas hermanas; dictamen humano | — |
| Multi-cliente y roles | `Cliente` (RFC) + selector global; `clienteId` en todos los módulos; roles capturista/glosador/gerente/cliente-consulta; alcance por cliente; bandeja de aprobaciones con audit trail | — |
| M3 / Data Stage | Importación en Pre-validador y Pre-Glosa; Pre-Glosa multipartida con cruces (origen-tratado, cuota por exportador, UMC/UMT, precios estimados, Apéndice 8); catálogo de reglas documentado (`docs/PREVALIDADOR_REGLAS.md`); combo de país; archivar al expediente | claves Anexo 22 agregadas sin fuente en repo → `cotejo: pendiente`; layout Data Stage |
| Anexo 24 | Pedimento-partida + parte con PEPS; BOM con mermas; retorno con constancias; activo fijo; submaquila; cierre mensual con candado; reporte Anexo 24 Excel/PDF; simulador de exposición | plazos I-bis/I-ter y estructura del reporte marcados pendientes de cotejo |
| Cotizador | Guardar/duplicar/versionar por cliente; PDF con folio, vigencia y fecha DOF del TC; IEPS por categoría; cuota automática por exportador; DTA por tipo (Art. 49 LFD); tabulador; escenarios; Excel | DTA por tipo: cotejo formal pendiente |
| MVE / Fiscal | E2 completo (método, incrementables/decrementables, vinculación, forma de pago, RFC, pesos); plantillas por proveedor; lote; vigencias; layout de trabajo; estado honesto. Fiscal: semáforo A/AA/AAA, avisos al calendario, conciliación vs Anexo 30, simulador de pérdida, descargos como flujo | requisitos sin texto en corpus → pendientes de cotejo; layout VUCEM no oficial |
| Origen / Cuotas | Salto arancelario por BOM, de minimis, acumulación, LVC/SA; cobertura de reglas (8544.30 sin regla, visible); certificado 9 elementos; portal de proveedores por token con alertas. Cuotas: tasas por exportador, pipeline UPCI, cotejo visible (0/40 cotejadas), alerta de elusión | reglas específicas y resoluciones UPCI: NO se inventaron; pipeline listo |
| Regulatorio / Calendario / Régimen | Watchdog DOF real (Diputados + índice DOF) filtrado por catálogo; digest semanal; acciones en un clic; severidad por monto; `/calendario` de obligaciones; asistente F4/F5/A3/RT; `/activo-fijo` | fechas del calendario base sin fuente → pendientes |
| Copilot / Risk / Expedientes | Copilot con contexto del cliente; precedentes por fracción (honesto: 0 verificados); pipeline de carga admin. Risk: folio, evidencia declarado→verificado, historial por cliente, cartera. Expedientes fusionados: checklist a)–h), glosa documental, retención 5 años, ZIP de auditoría | Notas Explicativas / jurisprudencia: no cargadas (licencia/fuente) |
| Defensa / Analytics / Fracciones | `/defensa` (Cumplimiento+Auditoría) con certificado de integridad; Analytics real que cuadra con Historial (ahorro/riesgo/equipo, por cliente, Excel); Fracciones ficha completa + árbol; Logística fuera del menú; "Tus datos" en Verificación | cifrado en reposo: pendiente de confirmar con Railway |
