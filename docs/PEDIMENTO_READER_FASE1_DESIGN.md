...
## Addendum 1.2 — decisiones de implementación (17-jul, tras aprobación + 3 adiciones)

Las 3 adiciones aprobadas quedaron así:
1. **País de origen → F3 VERIFICADO**: confirmado verbatim en criterios 551.21 v9.0 — *"En importación, clave del país de origen de las mercancías o donde se produjeron"* (campo propio, obligatorio salvo tránsitos; distinto de 551.22 vendedor). El mapper llena `paisOrigen` SOLO cuando 501.5 = 1 (catálogo verbatim: 1 Importación / 2 Exportación); en exportación el campo es destino y no se llena.
2. **Tolerancias aritméticas explícitas** (constantes en `layout-v9.ts`): contribuciones |Σ557(clave) − Σ510(clave)| ≤ n₅₅₇(clave) × **1 MXN** (importes enteros por spec); valores |Σ551.10 − Σ505.7| ≤ (n₅₅₁+n₅₀₅) × **0.01 USD**. Solo claves presentes en 557 se comparan (las exclusivamente globales como DTA no generan comparación). Exceso → advertencia de integridad, no fallo (la forma ya validó).
3. **Fracción inexistente = señal**: el parser extrae la fracción tal cual (valida solo forma A8); `validateFraction` fail-closed la convierte en `fraccionValida=false` → F6-CLA-01 puntúa 5 y el radar la destaca como `FRACCION_INEXISTENTE`. Verificado en paridad (fixture C, 99999999 → ROJO_CRITICO con hallazgo destacado).

Decisiones ancladas adicionales tomadas en implementación:
- **801.4 cuenta registros "sin considerar el registro 801"** (verbatim criterios 801.4) — así se valida.
- **500.6 (acuse)** solo se declara al eliminar/desistir/confirmar pago (verbatim criterios 500.6) — vacío en Pedimento Nuevo.
- **Unión partida↔hijos por llave** (fracción + número de partida de los campos 3-4 de 553/554/556/557), no por posición; el orden interno de registros NO se exige más allá de 500-abre / 800-cierra / 801-final. Partida duplicada o referencia a partida inexistente → fallo de archivo.
- **Número 15 reconstruido en formato de presentación del Anexo 22** (`AA AD PPPP NNNNNNN` con espacios, el que valida `validatePedimentoNumero`); requiere fecha de ENTRADA (506 tipo 1) y cruce del primer dígito del consecutivo con el último dígito del año (criterios 500.4) — si no cruza, no se reconstruye.
- **Claves crudas sin semántica**: contribuciones (Apéndice 12), identificadores (Apéndice 8) y países (Apéndice 4) se exponen con su clave tal cual. **Pendientes de vendorear con cotejo**: Apéndice 12 (para mapear IGI/DTA/IVA/IEPS del pre-validador) y Apéndice 4 (los fixtures asumen claves de país alfa-2 'US'/'CN'; si el catálogo vigente usa otra forma, F3 necesitará ese mapeo).
- **Corrección al diseño**: aridad real del 512 = 10 campos (el extracto inicial cortó en el salto de página).
- **Flag beta**: en producción el módulo requiere `PEDIMENTO_READER_ENABLED=true` (apagado por defecto); toda respuesta lleva `beta: true` + aviso de validación pendiente.
- **Paridad 1.4 (corrección declarada, no silenciosa)**: la primera predicción omitió F4-PAD-01 (73181501 exige sector 15 Anexo 10) y el formato con espacios del número (F8-DOC-01 disparaba por formato). Ambos eran comportamiento correcto del motor; se corrigieron predicción y mapper. Resultado final: 5/5 bandas conforme a lo declarado (VERDE / AMARILLO+ROJO / ROJO_CRITICO×2), tests 36/36, paridad 42/42.
